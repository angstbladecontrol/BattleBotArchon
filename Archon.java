...
public class Archon extends Bot { // declares bot class
	...
	public int getMove(BotInfo me, boolean shotOK, BotInfo[] liveBots, BotInfo[] deadBots, Bullet[] bullets) {
		...
		liveBots[x].getBulletsLeft();
		deadBots[x].getBulletsLeft();;
 ...
 */
		try {
			if (me.getY()<50){
				ai[0]-=1/Math.abs(me.getY()-RADIUS);
				ai[1]+=1/Math.abs((me.getY()-RADIUS)+40);
			}
			if(me.getY()>400){//me.getY-500. we get negative value. We must be within 100 to care. if <0 then that mean we are above. if >-100 then we are within 100 while approaching from above
				ai[1]-=1/Math.abs(500-me.getY()+RADIUS);
				ai[0]+=1/Math.abs((500-me.getY()+RADIUS)+40);
			}
			if(me.getX()<100){
				ai[2]-=1/Math.abs(me.getX()-RADIUS);
				ai[3]+=1/Math.abs((me.getX()-RADIUS)+40);
			}
			if(me.getX()>600){
				ai[3]-=1/Math.abs(700-me.getX()+RADIUS);
				ai[2]+=1/Math.abs((700-me.getX()+RADIUS)+40);
			}
		}
		catch(Exception e) {
		...
		if (target !=-1){//if it has a target
			...
			try {
				if(ady>adx){// y distance is greater so align by x movement
					 ...
					 */
					if (true){//snddeadcheck(me, 0, deadBots, liveBots[target])==false){
						if(dx>0){
					 ...
					 */
					if (true){//snddeadcheck(me, 1, deadBots, liveBots[target])){
						if (dy>0){
							ai[0]+=aggression/21.0;
						}
						else if (dy<0) {
